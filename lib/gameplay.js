...
export function createGameplay({ boat, camera, scene, water, segments, colliders, pickups, hud, baseRowerRot, particles, wake, world }) {
  ...
  function reset() {
    ...
    if (boat.userData.rower) {
      ...
      boat.userData.rower.position.set(boat.userData.seatPos.x, -0.42, boat.userData.seatPos.z);
    }
  ...
  function tick(dt) {
    ...
    if (state.phase === 'dying') {
      ...
      if (rower) {
        rower.position.y = -0.42 + state.deathT * 2.5 - 4.5 * state.deathT * state.deathT;
