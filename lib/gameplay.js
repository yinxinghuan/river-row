...
// Camera — preroll state (cinematic far-back so the rower is the focus)
const CAM_PREROLL_HEIGHT = 2.6;
const CAM_PREROLL_BACK   = 8.5;
const CAM_PREROLL_LOOK_AHEAD = 6.0;
const CAM_PREROLL_LOOK_Y = 0.7;

// Tap-to-start camera push-in
const PUSH_IN_T = 1.0;             // seconds to ease into chase
...
export function createGameplay({ boat, camera, scene, water, segments, colliders, pickups, hud, baseRowerRot, particles, wake, world }) {
  const state = {
    ...
    invulnT: 0,
    shakeT: 0,
    pushT: 0,                       // 0 → PUSH_IN_T: camera eases from preroll → chase
    // input
  ...
  function reset() {
    ...
    // Snap camera to preroll cinematic position so the rower is visible at load
    ...
    if (boat.userData.rower) {
      ...
      boat.userData.rower.position.set(boat.userData.seatPos.x, 0.13, boat.userData.seatPos.z);
    }
  ...
  function tick(dt) {
    if (state.phase === 'preroll') {
      ...
      // Hold camera at the cinematic preroll spot — show the rower from above-behind.
      const targetX = 0;
    ...
    if (state.phase === 'dying') {
      ...
      if (rower) {
        rower.position.y = 0.13 + state.deathT * 2.5 - 4.5 * state.deathT * state.deathT;
    ...
    // ── camera follow (always tracks boat XZ, smoothed) with a 1s push-in
    //    after the player taps to start
