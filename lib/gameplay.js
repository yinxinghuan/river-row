...
// ── camera (play) ─────────────────────────────────────────────────────────
// Two-stop lerp by speed: at base speed the camera sits close-ish; at boost
// it pulls back, lifts up, and looks further downstream so the player gets
// extra reaction time at the higher pace.
const CAM_HEIGHT_BASE      = 6.2;
const CAM_HEIGHT_BOOST     = 8.4;
const CAM_BACK_BASE        = 11.5;
const CAM_BACK_BOOST       = 15.5;
const CAM_LOOK_AHEAD_BASE  = 32.0;
const CAM_LOOK_AHEAD_BOOST = 46.0;
const CAM_LOOK_Y_BASE      = 0.1;
const CAM_LOOK_Y_BOOST     = -0.5;
const CAM_FOV_BASE         = 54;
const CAM_FOV_BOOST        = 64;
const CAM_LERP             = 7.0;
const CAM_BOOST_LERP       = 3.5;   // how fast the boost lerp tracks speed (per second)

// ── camera (preroll cinematic) ────────────────────────────────────────────
const CAM_PREROLL_HEIGHT     = 5.4;
const CAM_PREROLL_BACK       = 12.5;
const CAM_PREROLL_LOOK_AHEAD = 14.0;
const CAM_PREROLL_LOOK_Y     = 0.4;
    ...
    if (state.phase === 'play') {
      ...
      if (particles) {
        ...
        if (speed > 5) {
          ...
          const n = speed > 22 ? 3 : (speed > 16 || inLane) ? 2 : 1;
      ...
      if (wake) {
        const speed = Math.hypot(state.vx, state.vz);
        wake.scale.z = THREE.MathUtils.clamp(speed / 18, 0.5, 2.0);
      }
