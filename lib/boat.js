...
// ── wake — V-shaped foam arms diverging from the stern ──────────────────────
// Two tapered ribbons rotated ±14° outward from the boat's heading. Replaces
// the previous four parallel lines (read as "white stripes"). Far end fades
// + tapers narrower so the arms read as a real bow wake breaking outward.
...
function makeWakeArm(sign) {
  // Subtle hint of foam — the previous arms read as "two hard white
  // lines". Now: shorter, wider divergence, and capped at low alpha so
  // it just whispers behind the boat instead of drawing the eye.
  const L = 5.0;
  const W = 0.70;
  const geo = new THREE.PlaneGeometry(W, L, 1, 16);
  ...
  for (let v = 0; v < pos.count; v++) {
    const z = pos.getZ(v);
    const t = Math.max(0, Math.min(1, z / L));
    // taper x more aggressively + soft x-edge falloff so the strip
    // doesn't have hard side edges
    const lateralFade = 1 - Math.pow(Math.abs(pos.getX(v)) / (W*0.5), 2);
    pos.setX(v, pos.getX(v) * (1 - t * 0.65));
    // fast fade along the length; max alpha now ~0.22 instead of 0.95
    const a = Math.pow(1 - t, 1.6) * 0.22 * Math.max(0, lateralFade);
  ...
  m.rotation.y = sign * 0.32;              // wider divergence (~18°) so the arms feel like spread foam, not parallel lines
  m.position.set(sign * 0.22, 0.045, 1.0);
  return m;
}
