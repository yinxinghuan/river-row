...
//   -1.0  arms thrust forward (drive phase)
//   -0.55 neutral
//    0.1  arms pulled back (catch)
// Oar pose convention (rotation.x on oar pivot from rowlock):
//   -0.5  blade lifted out of water (recovery midpoint)
//    0.18 blade pushed deep through water (drive midpoint)
const ROW_CYCLE_BASE = 0.8;          // baseline row cadence (cycles/sec at rowFreq=0.5)
const ROW_CYCLE_MAX  = 2.6;          // max cadence when rowFreq=1
...
export function tickBoat(boat, t, dt) {
  ...
  // Continuous catch-drive-finish-recovery loop. armX traces a smoothed sine
  // between catch (back) and finish (forward), oarX inversely so the blade
  // pulls in time with the arm swing. Roll keeps a small dip on the drive
  // half so the blade reads as cutting through water.
  const sw = Math.sin(phase);              // -1..1, drives arms
  const cw = Math.cos(phase);              // 90° lead, drives blade depth
  // Arms swing from 0.1 (catch) to -1.0 (finish); mid-point -0.45.
  const armX = -0.45 - sw * 0.55;
  // Oar pivots from -0.5 (out of water on recovery) to 0.18 (deep on drive).
  // Use cw so the blade enters water just as arms start their backward pull.
  const oarX = -0.16 + cw * 0.34;
  const oarRoll = Math.max(0, cw) * 0.12;   // dip only when blade is in water
