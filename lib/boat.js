...
export function attachRower(boat, characterKey, CHARACTERS) {
  ...
  // Anchor LOW into the hull so the character "sits in" the boat instead of
  // sitting on top of it. We DON'T fold the legs — folding made the feet
  // float in mid-air. Instead, leave the character standing and drop them
  // far enough that the legs are entirely below the opaque floor box; the
  // boat hull occludes them and only the torso/arms/head show above the rim.
  //
  // Math: floor TOP is at boat-local y = 0.13 (floor.position.y=0.08 + h/2);
  // rim top is at y ≈ 0.43. We want the chest visible above the rim, so we
  // place the rower's HIP (local y = (shoeH+legH) × SCALE ≈ 0.55) at roughly
  // floor level → rower.y ≈ -0.42.
  rower.position.set(boat.userData.seatPos.x, -0.42, boat.userData.seatPos.z);
  // body tilt forward a small touch (over the oars)
  rower.rotation.x = 0.08;

  const rig = rower.userData.rig;
  if (rig) {
    // ── legs stay STANDING (no fold). They extend straight down from the hip
    //    and clip through / below the boat floor — hidden by the opaque hull.
