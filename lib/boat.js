...
// ── per-frame bob + stroke-driven row animation. ──
//
// Inputs come via boat.userData (set by gameplay each frame):
//   charging        bool      — finger is held, charging a stroke
//   chargeProgress  0..1      — how full the charge is
//   strokeProgress  0..1|null — how far through the post-release stroke animation
//                               (null if no stroke currently in flight)
//
// Arm pose convention (rotation.x on arm pivot):
//   -1.0  arms thrust forward (drive phase, just after release)
//   -0.55 neutral idle
//    0.1  arms pulled back (full charge ready to fire)
// Oar pose convention (rotation.x on oar pivot from rowlock):
//   -0.5  blade lifted out of water (charging up)
//   -0.16 neutral idle / blade just touching water
//    0.18 blade pushed deep through water (drive)
export function tickBoat(boat, t, dt) {
  // Gentle boat bob (Y only; rotations come from gameplay so they don't fight)
  boat.position.y = 0.05 + Math.sin(t * 1.4) * 0.025;

  const charging = !!boat.userData.charging;
  const chargeP  = boat.userData.chargeProgress || 0;
  const strokeP  = boat.userData.strokeProgress;     // null when idle

  let armX = -0.55;        // arm neutral
  let oarX = -0.16;        // oar neutral
  let oarRoll = 0;         // oar z-roll for blade dip

  if (strokeP !== null && strokeP >= 0 && strokeP < 1) {
    // ── stroke fire: 0..0.45 drive phase (thrust + blade in water),
    //                0.45..1 recovery (return to neutral).
    if (strokeP < 0.45) {
      const u = strokeP / 0.45;
      // arms swing from "pulled back" → "thrust forward"
      armX = 0.1 - u * 1.1;        // 0.1 → -1.0
      // oars drive: blade goes from lifted to deep-in-water
      oarX = -0.5 + u * 0.7;       // -0.5 → 0.2
      oarRoll = Math.sin(u * Math.PI) * 0.10;   // brief dip
    } else {
      const u = (strokeP - 0.45) / 0.55;
      // smooth return to neutral
      armX = -1.0 + u * 0.45;      // -1.0 → -0.55
      oarX =  0.2 - u * 0.36;      //  0.2 → -0.16
      oarRoll = 0;
    }
  } else if (charging) {
    // pull arms back + lift oars proportional to charge level
    armX = -0.55 + chargeP * 0.65;       // -0.55 → 0.1
    oarX = -0.16 - chargeP * 0.34;       // -0.16 → -0.5
    oarRoll = 0;
  }
