...
function makeWakeArm(sign) {
  const L = 9.0;
  const W = 0.85;
  const geo = new THREE.PlaneGeometry(W, L, 1, 18);
  geo.rotateX(-Math.PI / 2);
  // Move the pivot/origin to the boat-end of the strip so rotation.y around
  // the group origin pivots the FAR tail outward without sliding the head.
  geo.translate(0, 0, L / 2);
  // Per-vertex taper + foam fade: x narrows toward tail, alpha fades to zero.
  ...
  for (let v = 0; v < pos.count; v++) {
    ...
    // soft ease — bright near boat, fades fast at the tail
    const a = Math.pow(1 - t, 1.3) * 0.95;
  ...
  m.rotation.y = sign * 0.26;             // ~15° outward
  m.position.set(sign * 0.18, 0.045, 1.0); // pivot just behind the stern
  return m;
}
