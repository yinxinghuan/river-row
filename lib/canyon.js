...
function sandstoneBoulder(r) {
  let geo = new THREE.IcosahedronGeometry(r, 2);   // 320 faces — soft grain
  const pos0 = geo.attributes.position;
  for (let v = 0; v < pos0.count; v++) {
  ...
  geo = geo.toNonIndexed();
  geo.computeVertexNormals();
  paintFacesByHeight(geo, 0xa0533a, 0xc77b54, 0.020);
  const mat = new THREE.MeshLambertMaterial({ vertexColors: true, flatShading: true });
  applyCurve(mat);
  const m = new THREE.Mesh(geo, mat);
  m.castShadow = true;
  m.receiveShadow = false;          // avoid the dark self-shadow stripes
  return m;
}
