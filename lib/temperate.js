...
function mossyBoulder(r) {
  // detail=1 → 80 faces instead of 20 → finer facets, softer facet-edge
  // contrast (user reported "cracks" with detail=0 + 0.18 jitter)
  ...
  geo = geo.toNonIndexed();                        // crack-free per-face paint
  geo.computeVertexNormals();
  paintFacesByHeight(geo, 0x7eb476, 0x959990, 0.04);   // moss / stone gray (lower jitter)
  const mat = new THREE.MeshLambertMaterial({ vertexColors: true, flatShading: true });
  applyCurve(mat);
  const m = new THREE.Mesh(geo, mat);
