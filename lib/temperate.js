...
function bankSlab(w, h, l) {
  // Tiered stacked-box bank — same crack-free recipe as the canyon walls.
  // Two-three blocks, each one a clean BoxGeometry with paintBoxByFace, no
  // vertex jitter (which produced visible facet seams on the previous slab).
  const g = new THREE.Group();
  const TOP  = new THREE.Color(0x84b075);
  const SIDE = new THREE.Color(0xb59465);
  const tiers = 2 + Math.floor(Math.random() * 2);
  let yCursor = -h * 0.5;
  for (let i = 0; i < tiers; i++) {
    const taper = i / Math.max(1, tiers - 1);
    const tw = w * (1.0 - taper * 0.12 + (Math.random()-0.5) * 0.10);
    const th = h / tiers * (0.85 + Math.random() * 0.35);
    const td = l * (0.92 + Math.random() * 0.14);
    ...
    box.position.set(
      (Math.random() - 0.5) * w * 0.16,
      yCursor + th * 0.5,
      (Math.random() - 0.5) * l * 0.06,
    );
    box.rotation.y = (Math.random() - 0.5) * 0.18;
