...
function mossyBoulder(r) {
  // detail=2 → 320 faces. Per-face shading on so many small triangles reads
  // as a smooth-with-grain rock rather than the chunky-faceted "cracked" look.
  let geo = new THREE.IcosahedronGeometry(r, 2);
  const pos0 = geo.attributes.position;
  for (let v = 0; v < pos0.count; v++) {
    pos0.setY(v, pos0.getY(v) * 0.6 + r * 0.05);
    pos0.setX(v, pos0.getX(v) + (Math.random()-0.5) * 0.05);
    pos0.setZ(v, pos0.getZ(v) + (Math.random()-0.5) * 0.05);
  }
  geo = geo.toNonIndexed();
  geo.computeVertexNormals();
  paintFacesByHeight(geo, 0x7eb476, 0x959990, 0.025);
  const mat = new THREE.MeshLambertMaterial({ vertexColors: true, flatShading: true });
  applyCurve(mat);
  const m = new THREE.Mesh(geo, mat);
  m.castShadow = true;
  m.receiveShadow = false;          // don't self-shadow — the dark stripes read as cracks
  return m;
}
...
function contactFoam(r) {
  const inner = r * 0.78;
  const outer = r * 1.55;
  const geo = new THREE.RingGeometry(inner, outer, 22);
  geo.rotateX(-Math.PI / 2);
  // soft alpha falloff via vertex color (inner = bright, outer = transparent-ish)
  const pos = geo.attributes.position;
  const col = new Float32Array(pos.count * 3);
  for (let v = 0; v < pos.count; v++) {
    const x = pos.getX(v), z = pos.getZ(v);
    const d = Math.sqrt(x*x + z*z);
    const t = (d - inner) / (outer - inner);     // 0 at inner, 1 at outer
    const a = 1 - t;                              // brighter near rock
...
// ── lily pads (small clusters floating on water — pickup) ────────────────────
export function buildLilyPads({ count = 14, zStart = -130, zEnd = 130 } = {}) {
  const group = new THREE.Group();
  const length = zEnd - zStart;
  for (let i = 0; i < count; i++) {
    // Bias clusters AWAY from far-bank zones so they live in the play lane;
    // these are pickups, the player should be tempted to dip toward them.
    const cx = (Math.random() < 0.5 ? -1 : 1) * (0.5 + Math.random() * (RIVER_HALF - 2));
    ...
    cluster.userData.pickup = { r: 1.3, value: 10 };
    group.add(cluster);
  }
  return group;
}
...
function padCluster(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (Math.random() < 0.42) {
      let bg = new THREE.IcosahedronGeometry(0.12 + Math.random()*0.05, 0);
      bg = bg.toNonIndexed();
