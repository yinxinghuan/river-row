...
export function buildWater({ width = 60, length = 700 } = {}) {
  ...
  // Water uses scene fog (= sky horizon colour) so the river dissolves into
  // the sky's horizon glow at distance with no hard seam. The shader's
  // distance fade is gone — fog handles it now, and the colour automatically
  // tracks whichever biome is overhead.
