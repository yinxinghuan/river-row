...
const LANE_INFLUENCE = 5.5;         // ±half-width of the "current pulls you in" zone
...
export function buildWater({ width = 60, length = 700 } = {}) {
  ...
  // Three-stop water depth shader:
  //   0..8 m    foreground cyan (base vertex colour)
  //   8..70 m   blend toward deep teal — gives the "river has depth" read
  //   70..170 m blend toward horizon haze (= scene.fog.colour exactly) so
  //             the water-sky junction stays seamless.
  // uHorizonColor is updated in setSegment per-biome so canyon water fades
  // to warm haze, temperate to pale cyan.
  const uHorizonColor = { value: new THREE.Color(0xdce8e3) };
  ...
  applyCurve(waterMat, {
    vertexInsert: `
      ...
      transformed.y += _wave;
    `,
    fragmentHeader: `uniform vec3 uHorizonColor;`,
    fragmentInsert: `
      vec3 _deepWater = vec3(0.082, 0.282, 0.368);
      float _t1 = smoothstep(10.0, 65.0, vCamDist);
      float _t2 = smoothstep(75.0, 170.0, vCamDist);
      vec3 _mid = mix(gl_FragColor.rgb, _deepWater, _t1 * 0.38);
      gl_FragColor.rgb = mix(_mid, uHorizonColor, _t2);
    `,
  ...
  // The far-distance horizon target must match scene.fog.colour exactly so
  // the river dissolves into the sky's horizon band with no seam. Values
  // mirror the fog colours hard-coded in segments.js.
  const HORIZON_TEMPERATE = new THREE.Color(0xdce8e3);
  const HORIZON_CANYON    = new THREE.Color(0xf2deba);
  group.userData.setSegment = (t) => {
    waterMat.color.copy(TINT_TEMPERATE).lerp(TINT_CANYON, t);
    foamMat.opacity = 0.55 - t * 0.20;
    uHorizonColor.value.copy(HORIZON_TEMPERATE).lerp(HORIZON_CANYON, t);
  };

  group.userData.isInLane = (worldX) => Math.abs(worldX - laneX) < LANE_HALF_WIDTH;
  group.userData.laneX = () => laneX;
  // Unitless lateral pull in [-1, +1] toward the lane centreline. Zero outside
  // the influence zone; zero at the exact centre; peak magnitude (±1) at the
  // edge of influence. Gameplay multiplies this by its own LANE_PULL_ACCEL.
  group.userData.currentPull = (worldX) => {
    const d = worldX - laneX;
    const absD = Math.abs(d);
    if (absD >= LANE_INFLUENCE) return 0;
    return -Math.sign(d) * (absD / LANE_INFLUENCE);
  };
