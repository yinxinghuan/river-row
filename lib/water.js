...
export function buildWater({ width = 60, length = 260 } = {}) {
  ...
  const base = new THREE.Color(0x3aaec2);   // 湖青绿
  ...
  // L1 waves injected before the curve drop. curveStrength=0 keeps the
  // water plane FLAT in world space — without this the curve pulls distant
  // water below the camera's view line at d≳80 and the river visually
  // ends mid-screen ("水消失").
  applyCurve(waterMat, {
    curveStrength: 0,
    vertexInsert: `
  ...
  applyCurve(foamMat, { curveStrength: 0 });
  ...
  applyCurve(glintMat, { curveStrength: 0 });
  ...
  const TINT_CANYON    = new THREE.Color(0x2c4666);            // saturated dark blue — must stay visibly cool against the warm canyon fog/sky
